...
@st.cache_resource
def load_assets():
    # 'SolaimanLipi' ফন্ট যা বাংলা টেক্সট অ্যালাইনমেন্ট এবং দেখার জন্য সেরা
...
if uploaded_file:
    ...
    # Font Setup with SolaimanLipi (Correct Alignment)
    try:
        font_lg = ImageFont.truetype(io.BytesIO(font_data), 85) # বড় নাম
        font_md = ImageFont.truetype(io.BytesIO(font_data), 50) # স্লোগান
        font_sm = ImageFont.truetype(io.BytesIO(font_data), 40) # এলাকা
    except:
        font_lg = font_md = font_sm = ImageFont.load_default()

    # Drawing Bengali Text (Center Aligned with Anchor "mm")
    # নাম
    draw.text((540, 820), user_name, fill="#ffd700", font=font_lg, anchor="mm")
